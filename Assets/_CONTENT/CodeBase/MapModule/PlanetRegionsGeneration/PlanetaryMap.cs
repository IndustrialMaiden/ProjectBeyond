...
{
    public class PlanetaryMap
    {
        private const int LloydRelaxationsNumber = 5;
        
        public Graph.Graph Graph { get; }

        public PlanetaryMap(int regionsCount, RandomGenerator random, float width, float height, float pointSpacing, float noiseScale, float noiseResolution)
        {
            ...
            Graph = new Graph.Graph(points, voronoi, (int)width, (int)height, random, pointSpacing, noiseScale, noiseResolution);
